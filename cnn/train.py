...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250,
          batch_size=128, noise_size=100, num_epochs=10, train_loader=None, device=None, folder=None):
    """
    Train loop for GAN.
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            logits_fake2 = D(G(g_fake_seed).view(batch_size, input_channels, img_size, img_size))
            ...
            if iter_count % show_every == 0:
                ...
                plt.suptitle('Epoch/Iter:{}/{}, D: {:.4}, G:{:.4}'.format(epoch, iter_count, d_error.item(),
                                                                          g_error.item()))
                show_images(imgs_numpy[0:16], color=input_channels != 1)
                plt.show()
                # Save your results
                # plt.savefig(folder + '/I_{}.png'.format(iter_count))
                print()
            iter_count += 1
